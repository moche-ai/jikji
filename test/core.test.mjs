...
test('write(self) → approved → searchable round-trip', async () => {
  ...
  try {
    ...
    const s = await core.search(ctxOf('nsA'), { need: '내가 좋아하는 커피' });
...
test('injection 패턴 write → quarantined → 검색 미노출', async () => {
  const { core, cleanup } = freshCore();
  try {
    core.ensureTenant('nsA', 'owner:a');
    ...
    assert.equal((await core.search(ctxOf('nsA'), { need: 'api key' })).results.length, 0);
  } finally { cleanup(); }
});

test('external author_type(ctx 봉인) → quarantined', async () => {
  const { core, cleanup } = freshCore();
  try {
    core.ensureTenant('nsA', 'owner:a');
...
test('secret write → 거부 (zero-width 회피도 정규화로 차단, P0-7)', async () => {
  const { core, cleanup } = freshCore();
  try {
    core.ensureTenant('nsA', 'owner:a');
...
test('expected_version CAS: update 는 버전 필수, stale=409 (P0-1)', async () => {
...
test('purge: forget → 전 활성 저장소 참조 0 (P0-4)', async () => {
  const { store, core, cleanup } = freshCore();
  try {
    core.ensureTenant('nsA', 'owner:a');
    ...
    assert.equal((await core.search(ctxOf('nsA'), { need: '지워질 기억' })).results.length, 0);
  } finally { cleanup(); }
});

test('invalidate: 철회 리비전 → 검색 미노출', async () => {
  const { core, cleanup } = freshCore();
  try {
    core.ensureTenant('nsA', 'owner:a');
    const w = core.write(ctxOf('nsA'), { text: '틀린 사실이었다' });
    assert.equal(core.invalidate(ctxOf('nsA'), { fact_id: w.fact_id }).status, 'retracted');
    assert.equal((await core.search(ctxOf('nsA'), { need: '틀린 사실' })).results.length, 0);
  } finally { cleanup(); }
});

test('stale pending 승인 → CAS 409 (P0-1: base_version 보존)', async () => {
  const { store, core, cleanup } = freshCore();
  try {
    core.ensureTenant('nsA', 'owner:a');
...
test('write-time CAS: update expectedVersion 불일치 즉시 409 (P0-1 제출검증)', async () => {
  const { store, core, cleanup } = freshCore();
  try {
    core.ensureTenant('nsA', 'owner:a');
...
test('outbox 멱등 drain (P0-3: lease claim) — 재호출 시 재처리 0', async () => {
  const { store, core, cleanup } = freshCore();
  try {
    core.ensureTenant('nsA', 'owner:a');
...
test('authorType 누락 → fail-closed (P1)', async () => {
  const { core, cleanup } = freshCore();
  try {
    core.ensureTenant('nsA', 'owner:a');
...
test('scope 강제 + 빈 질의 422', async () => {
  const { core, cleanup } = freshCore();
  try {
    core.ensureTenant('nsA', 'owner:a');
    assert.throws(() => core.write({ namespaceId: 'nsA', scopes: ['retrieve'], authorType: 'self' }, { text: 'x' }), (e) => e.code === 403);
    await assert.rejects(core.search(ctxOf('nsA'), { need: '   ' }), (e) => e.code === 422); // 빈 질의 랜덤 top-k 방지
  } finally { cleanup(); }
});
