...
test('update → supersede: 이전 head=superseded, 새 head=active, 대체관계 기록', async () => {
...
test('모순검출(feature flag): 유사·상충 → disputed 양쪽 보존 + conflict_set', async () => {
  const { core, cleanup } = freshCore();
  try {
    ...
    const s = await core.search(ctxOf('nsA'), { need: '연락처' });
...
test('모순검출 기본 OFF: 플래그 없으면 disputed 안 만듦(기준선 불변)', async () => {
  const { core, cleanup } = freshCore();
  try {
...
test('pending inbox + review: assistant 자동추출 → pending → approve 후 검색 노출', async () => {
  const { core, cleanup } = freshCore();
  try {
    core.ensureTenant('nsA', 'owner:a');
    ...
    // 승인 전 검색 미노출
    assert.equal((await core.search(ctxOf('nsA'), { need: '데드라인' })).results.length, 0);
    ...
    // 승인 후 노출
    assert.ok((await core.search(ctxOf('nsA'), { need: '데드라인' })).results.length >= 1);
  } finally { cleanup(); }
});

test('review reject: 색인 안 됨', async () => {
  const { core, cleanup } = freshCore();
  try {
    core.ensureTenant('nsA', 'owner:a');
    const w = core.write(ctxOf('nsA', 'assistant'), { text: '거절될 자동추출' });
    core.review(ctxOf('nsA'), { revision_id: w.revision_id, decision: 'reject' });
    assert.equal((await core.search(ctxOf('nsA'), { need: '거절' })).results.length, 0);
    assert.equal(core.pending(ctxOf('nsA')).items.length, 0);
  } finally { cleanup(); }
});

test('md 임포트 → fact 분해 + export 라운드트립', async () => {
  const { core, cleanup } = freshCore();
  try {
    core.ensureTenant('nsA', 'owner:a');
    const md = `# 프로필\n- 이름은 장주\n- 커피는 아메리카노\n\n## 선호\n1. 아침형 인간\n2. 등산을 좋아함\n`;
    ...
    const s = await core.search(ctxOf('nsA'), { need: '등산' });
...
test('존재하지 않는 fact update → 404 (클라이언트 지정 id 신규생성 금지)', async () => {
  const { core, cleanup } = freshCore();
  try {
    core.ensureTenant('nsA', 'owner:a');
...
test('approved 재-review 차단(전이 forbidden 409)', async () => {
  const { store, core, cleanup } = freshCore();
  try {
    core.ensureTenant('nsA', 'owner:a');
...
test('IDOR: pending/review 도 namespace 격리', async () => {
  const { core, cleanup } = freshCore();
  try {
...
test('disputed fact 한쪽 forget → conflict 해소(생존자 active 복귀) + 0참조', async () => {
  const { store, core, cleanup } = freshCore();
  try {
    core.ensureTenant('nsA', 'owner:a', { auto_approve: true, default_no_train: true, contradiction_detection: true });
...
test('pending update 승인 → 이전 head superseded', async () => {
  const { store, core, cleanup } = freshCore();
  try {
...
test('md 임포트도 secret/injection 게이트 통과', async () => {
  const { core, cleanup } = freshCore();
  try {
    core.ensureTenant('nsA', 'owner:a');
    ...
    const all = (await core.search(ctxOf('nsA'), { need: 'token instructions' })).results;
    assert.ok(!all.some((r) => /ignore|reveal|instruction/i.test(r.fact)), 'injection 파생 fact 는 검색 후보에 없음(격리)');
    assert.ok((await core.search(ctxOf('nsA'), { need: '정상 사실' })).results.some((r) => r.fact.includes('정상 사실')));
  } finally { cleanup(); }
});
